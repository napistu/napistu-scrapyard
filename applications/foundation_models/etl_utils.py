...
def _scgpt_extract_weights(
  ...
  for layer_idx in range(n_layers):
    ...
    attention_layers.append(
      AttentionLayer(
        layer_idx=layer_idx, W_q=w_q, W_k=w_k, W_v=w_v, W_o=w_o
      )
    )
...
def _scprint_load_gene_annotations(model: Any) -> pd.DataFrame:
  ...
  try:
    ...
    ensembl_to_symbol = (
      all_genes_df.set_index("ensembl_gene_id")["symbol"].to_dict()
    )
...
def _aidocell_load_model_full(model_class: Any) -> Tuple[Any, pd.DataFrame, Dict]:
  ...
  logger.info("Formatting model metadata")
  model_metadata = _aidocell_format_metadata(model, model_class_name=model_class.__name__)
